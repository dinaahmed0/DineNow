...
export default function ReservationPage() {
  ...
  const [orderedFood, setOrderedFood] = useState<any[]>([]);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload = {
        restaurantId,
        startDateTime: toApiLocalDateTime(start),
        endDateTime: toApiLocalDateTime(end),
        numberOfGuests,
  ...
  const handleFoodOrderUpdate = useCallback((foodItems: any[]) => {
    setOrderedFood(foodItems);
  }, []);

  const handlePaymentComplete = (_payment: unknown) => {
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto">
                          {orderedFood.map((item, idx) => (
                            <div key={item.id} className="flex justify-between items-center text-sm">
                              <div className="flex items-center gap-2">
                                <span className="font-medium text-gray-600 min-w-[24px]">{item.quantity}×</span>
