...
export async function registerUser(userData: Omit<RegisterRequest, 'role'>): Promise<RegisterResponse> {
  if (USE_MOCK_MODE) {
    ...
    console.log('MOCK MODE: Simulating registration with data:', userData);
...
export async function loginUser(userData: LoginRequest): Promise<LoginResponse> {
  if (USE_MOCK_MODE) {
    ...
    console.log('MOCK MODE: Simulating login with data:', userData);
...
export async function verifyEmail(userData: VerifyEmailRequest): Promise<VerifyEmailResponse> {
  if (USE_MOCK_MODE) {
    console.log('MOCK MODE: Simulating email verification with data:', userData);
...
export async function refreshToken(userData: RefreshTokenRequest): Promise<RefreshTokenResponse> {
  if (USE_MOCK_MODE) {
    ...
    if (userData.refreshToken && userData.refreshToken.startsWith('mock-refresh-token')) {
      return {
        statusCode: MOCK_STATUS_CODE,
        meta: 'mock',
        succeeded: true,
